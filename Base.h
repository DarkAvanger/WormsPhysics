#pragma once
//IGNORE not physics related, used for this game simple/easy to use, used instead of SDL for retro style
...
public:
	...
	{
		m_bAtomActive = true;
		std::thread t = std::thread(&Base::GameThread, this);
...
protected: // Audio Engine =====================================================================
		...
		unsigned int nBlocks = 8, unsigned int nBlockSamples = 512)
	{
		...
		m_AudioThread = std::thread(&Base::AudioThread, this);
	...
	{
		((Base*)dwInstance)->waveOutProc(hWaveOut, uMsg, dwParam1, dwParam2);
	}
...
std::atomic<bool> Base::m_bAtomActive(false);
std::condition_variable Base::m_cvGameFinished;
std::mutex Base::m_muxGame;
